...
N_LAYER = 2
N_EPOCHS = 100
N_CHARS = 128 #是字典长度，因为是英文名字，所以是根据ASCII码设置的
USE_GPU = True
...
class NameDataset():         #处理数据集
    def __init__(self, is_train_set=True):
        ...
        with gzip.open(filename, 'rt') as f:    #打开压缩文件并将变量名设为为f
            ...
            rows = list(reader)                 #rows=(name,country)
    ...
    def __getitem__(self, index): #提供索引访问的方法
        return self.names[index], self.country_dict[self.countries[index]] #country和index就是key和value
    ...
    def idx2country(self,index):            #返回索引对应的国家名
...
class RNNClassifier(torch.nn.Module):
    def __init__(self, input_size, hidden_size, output_size, n_layers=1, bidirectional=True):
        ...
        self.n_directions = 2 if bidirectional else 1 #bi表示双向
    ...
    def forward(self, input, seq_lengths):
        input = input.t()               #t:transpose转置。input shaoe :  Batch x Seq -> S x B 用于embedding
...
def make_tensors(names, countries):     #处理名字ASCII码 重新排序的长度和国家列表
    sequences_and_lengths= [name2list(name) for name in names] #把每个名字变成几个ASCII码，存成列表；name2list返回arr和len(arr)
